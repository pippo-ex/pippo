defmodule Pippo.Interpreter.Evaluator do
  alias Pippo.Interpreter.Ast.{
    ArrayLiteral,
    BlockStatement,
    BooleanLiteral,
    CallExpression,
  ...
  alias Pippo.Interpreter.Evaluator.Builtins
  ...
  def eval(%BooleanLiteral{} = ast_node, env) do
    value = %Boolean{value: ast_node.value}
    {value, env}
  end
  ...
  defp do_eval_program([statement | rest], env, _last_evaluated) do
  ...
  defp eval_index_expression(left, _),
    do: error("index operator not support: #{Object.type(left)}")
  ...
  defp eval_hash_pair([], env, evaluated_pairs) do
    hash = %Hash{pairs: evaluated_pairs}
    {hash, env}
  end

  defp eval_hash_pair([pair | rest], env, evaluated_pairs) do
    {key, value} = pair
